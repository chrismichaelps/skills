...
Defines how you talk about the code itself:

- **Module** — anything with an interface and implementation (function, class, package, tier-slice). Scale-agnostic.
- **Interface** — everything a caller needs to know: types, invariants, error modes, ordering, config. Not just the signature.
- **Depth** — leverage: does a small interface hide a lot of behavior? Deep = high leverage. Shallow = interface nearly as complex as the implementation.
- **Seam** — where an interface lives; where behavior can change without editing in place. One adapter = hypothetical seam. Two adapters = real seam.
- **Locality** — bugs and changes concentrated in one place (good) vs. scattered across N callers (bad). Depth grants locality.
- **Deepening** — the act of extracting a deeper interface from a shallow module to improve leverage and locality.
...
When exploring a codebase, do not run rigid heuristics. Read it organically and note where friction appears:

- Where does understanding one concept require bouncing between many small modules? → Shallow modules leaking across seams.
- Where is the interface nearly as complex as the implementation? → Low leverage.
- Where have pure functions been extracted for testability, but real bugs hide in how they're called? → Fake seam, no locality.
- Where do modules leak across their boundaries? → High adaptation cost.
- Which parts are untested or hard to test through their interface? → Interface is fighting the domain.

**The Deletion Test:** Imagine deleting a module. Does complexity vanish (it was a pass-through), or reappear across N callers (it was earning its keep)? "Complexity concentrates when deleted" is the signal — that module deserves deepening.
...
---
### Module
- **Definition:** Any unit with an interface and an implementation — function, class, package, or tier-slice. Scale-agnostic.
...
- **Canonical name:** Seam
- **Not:** "Boundary", "Interface" (interface is what lives at the seam, not the seam itself)
- **Example:** PaymentGateway seam — Stripe adapter (prod) + MockPayment adapter (test)

### Depth
- **Definition:** Leverage: how much behavior is hidden behind how small an interface. Deep = small interface, large implementation. Shallow = interface nearly as complex as implementation.
- **Canonical name:** Depth (or DEPTH_SCORE when referring to the metric)
- **Not:** "Abstraction level", "complexity" (complexity is a cost; depth is a quality)

### Deepening
- **Definition:** The act of extracting a deeper interface from a shallow module to improve leverage and locality.
